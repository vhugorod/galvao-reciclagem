...
class TH_PortfolioSlider extends ZnElements
{
	...
	function element()
	{
		...
		<div class="<?php echo implode(' ', $classes); ?>" <?php zn_the_element_attributes($options); ?>>

			<div class="fake-loading loading-1s"></div>

			<div class="bgback"></div>
			<?php
				$psl_fullscreen = $this->opt( 'psl_fullscreen', '0' ) == 1 ? 'psl--fullscreen' : '';
				$psl_height = (int)$this->opt( 'ww_height' ) ? 'psl--height' : '' ;
			?>
			<div class="kl-slideshow-inner portfolio-slider-frames psl__wrapper <?php echo esc_attr( $hclass );?> <?php echo esc_attr( $psl_fullscreen ); ?> <?php echo esc_attr( $psl_height ) ?>">

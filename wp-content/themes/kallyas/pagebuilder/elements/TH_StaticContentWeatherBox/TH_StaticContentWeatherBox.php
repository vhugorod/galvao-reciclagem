...
class TH_StaticContentWeatherBox extends ZnElements {
	...
	function element() {
		...
		$obj_parallax__class   = '';
		$obj_parallax__attribs = '0';

		if ( $is_screffect ) {
			$obj_parallax__class   = 'js-doObjParallax zn-objParallax';
			$obj_parallax__attribs = '-5';
		} ?>
		<div class="kl-slideshow static-content__slideshow sc--weatherbox <?php echo esc_attr( $style ); ?> <?php echo esc_attr( $uid ); ?> <?php echo esc_attr( $bm_class ); ?> <?php zn_the_element_classes( $this->data['options'] ); ?>">
		...
		$static_content_height       = ( (int)$this->opt( 'ww_height' ) ? 'static-content--height' : '' ); ?>
			...
			?>
				...
				<div class="static-content__inner container">
					<div class="kl-slideshow-safepadding sc__container <?php echo esc_attr( $obj_parallax__class ); ?>" data-rellax-speed="<?php echo esc_attr( $obj_parallax__attribs ); ?>">
		...
		if ( $this->opt( 'scw_enable', '1' ) && $this->opt( 'scw_apikey' ) != '' ): ?>

								<div class="sc__weather" id="sc__weather_<?php echo esc_attr( $uid ); ?>" data-apikey="<?php echo esc_attr( base64_encode( $this->opt( 'scw_apikey', '' ) ) ); ?>" data-location="<?php echo esc_attr( $this->opt( 'scw_location', '' ) ); ?>" data-unit="<?php echo esc_attr( $this->opt( 'scw_unit', 'imperial' ) ); ?>">
		...
		$options = array(
			...
			'general'  => array(
				...
				'options' => array(
					...
					array(
						"name"        => __( "Open Weather Map Api Key", 'zn_framework' ),
						"description" => __( "Add your api key. You can get one <a href='https://openweathermap.org/' target='_blank'>here</a>, you will need to create an account and generate an api key ", 'zn_framework' ),
						"id"          => "scw_apikey",
						"std"         => "",
						"type"        => "text",
					...
					array(
						...
						"description" => __( "Add your city name or geo coords to improve accuracy", 'zn_framework' ),
						"id"          => "scw_location",
						"std"         => "Chicago",
						"type"        => "text",
						"placeholder" => "eg: London, GB",
					),
					array(
						'id'          => 'scw_unit',
						'name'        => 'Display unit',
						...
						'std'         => 'imperial',
						"options"     => array(
							"imperial" => __( "Farenheit", 'zn_framework' ),
							"metric"   => __( "Celsius", 'zn_framework' ),
						),
					),

...
//#! Show tags
$sp_tags = get_post_meta( get_the_ID(), 'sp_show_tags', true );
if( empty( $sp_tags ) || ( $sp_tags == 'default' ) ) {
	$sp_tags = zget_option( 'ports_show_tags', 'portfolio_options', false, 'no' );
}
if( 'yes' == $sp_tags ) {
	echo '
	<li class="portfolio-item-details-cat clearfix">
		<span class="portfolio-item-details-label">' . __( "TAGS ", 'zn_framework' ) . '</span>
		<span class="portfolio-item-details-item">' . get_the_term_list( get_the_ID(), 'portfolio_tags', '', ' , ', '' ) . '</span>
	</li>';
}
echo '</ul>';

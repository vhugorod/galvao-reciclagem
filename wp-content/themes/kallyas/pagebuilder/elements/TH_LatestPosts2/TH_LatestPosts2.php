...
class TH_LatestPosts2 extends ZnElements
{
	...
	function element()
	{
		...
		?>
			...
			<div class="latest_posts style3 latest_posts--style2 latest_posts2 clearfix <?php echo implode(' ', $elm_classes); ?>" <?php echo zn_get_element_attributes($options); ?>>
					...
					while ( have_posts() ) {
						...
						if( $this->opt('show_img','yes') == 'yes' ){
							...
							if ( ! empty ( $f_image ) ) {
								...
								echo '<a href="' . get_permalink() . '" class="hoverBorder pull-left latest_posts2-thumb"><img src="'. esc_url( $image['url'] ) . '" width="' . esc_attr( $image['width'] ) . '" height="' . esc_attr( $image['height'] ) . '" alt="' . esc_attr( $alt ) . '" title="'. esc_attr( $title ) .'" class="latest_posts2-thumb-img"/></a>';
							}
						}

...
class TH_StepsBox extends ZnElements
{
	...
	function css(){
		...
		foreach ($boxes as $key => $box) {
			if( ! empty( $box['stp_box_color'] ) ){
				...
				$css .= "#{$uid}.process_steps--style1 .process_steps__step:nth-child({$index}) { background-color: {$bg_color}; }";
				$css .= "#{$uid}.process_steps--style1 .process_steps__step:nth-child({$index}):after { border-left-color: {$bg_color}; }";
			}
		}
	...
	function element()
	{
		...
		<?php echo '<div id="'.$this->data['uid'].'" class="process_steps '.implode(' ', $classes).'" '.$attributes.'>'; ?>
			...
			<div class="process_steps__step process_steps__intro process_steps__height">
				...
				if ( ! empty ( $stp_text_link ) && ! empty ( $stp_link['start'] ) ) {
					printf( '%s', $stp_link['start'] . $stp_text_link . ' +' . $stp_link['end'] );
				}
				?>
				</div>
			</div>
	...
	function options()
	{
		$extra_options = array (
			...
			"subelements"    => array (
				...
				array (
					...
					"description" => __( "Select the desired animation for this step.", 'zn_framework' ),
					"id"          => "stp_single_anim",
					"type"        => "select",
					"std"         => "tada",
					"options"     => array (
		...
		$options = array(
			...
			'help' => znpb_get_helptab( array(
				'video'   => sprintf( '%s', esc_url('https://my.hogash.com/video_category/kallyas-wordpress-theme/#f4nKO-461X0') ),
